...
@Component({
  ...
  templateUrl: './cuenta-list.tab.html',
})
export class CuentaListTab implements OnInit {
  ...
  readonly filas = signal<CuentaBancaria[]>([]);
  ...
  abrirFormulario(item?: CuentaBancaria): void {
    this.dialog
      .open(CuentaFormDialog, { data: { item }, width: '640px' })
      .afterClosed()
      .subscribe((guardado) => {
        if (guardado) {
          this.snackbar.open('Cuenta guardada', 'OK', { duration: 3000 });
          this.cargar();
        }
      });
  }

  async verSaldo(item: CuentaBancaria): Promise<void> {
    const info = await firstValueFrom(this.servicio.saldo(item.id));
    this.snackbar.open(
      `Saldo actual de ${info.banco} ${info.numero_cuenta}: ${this.money.transform(info.saldo_actual)}`,
      'OK',
      { duration: 6000 },
    );
  }

  eliminar(item: CuentaBancaria): void {
    this.dialog
      .open(ConfirmDialog, {
        data: {
          titulo: 'Eliminar cuenta bancaria',
          mensaje: `¿Eliminar la cuenta "${item.banco} ${item.numero_cuenta}"? El registro quedará inactivo.`,
        },
      })
      .afterClosed()
