...
import { SucursalesService, etiquetaTipoSucursal } from './sucursales.service';
...
export class SucursalListPage implements OnInit {
  ...
  readonly columnas = ['nombre', 'tipo', 'direccion', 'telefono', 'responsable', 'estado', 'acciones'];
  readonly filas = signal<Sucursal[]>([]);
  ...
  etiquetaTipo(tipo: string): string {
    return etiquetaTipoSucursal(tipo);
  }
  ...
  abrirFormulario(item?: Sucursal): void {
    this.dialog
      .open(SucursalFormDialog, { data: { item }, width: '640px' })
      .afterClosed()
      .subscribe((guardado) => {
        if (guardado) {
          this.snackbar.open('Sucursal guardada', 'OK', { duration: 3000 });
          this.cargar();
        }
      });
  }
    ...
    this.dialog
      .open(ConfirmDialog, {
        data: {
          ...
          mensaje: `¿Eliminar la sucursal "${item.nombre}"? El registro quedará inactivo.`,
        },
      })
      .afterClosed()
